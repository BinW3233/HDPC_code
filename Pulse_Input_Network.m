...
%% Specify the pulse inputs
% Keep x and y as p-by-1 vectors so w*x and v*y produce nsize-by-1 inputs.
x = zeros(p, 1);
y = zeros(p, 1);
x(1) = amp;
y(1) = amp;
...
%% Save the data
outputFile = ['TDcode_longtxy_', 'mu_', num2str(mu), '_b_', ...
    strtrim(num2str(b)), '_deltat_', num2str(delta_t), '_p_', ...
    num2str(p), '.mat'];
save(outputFile, "t_tot", "ht", "tx", "hx", "ty", "hy", "jmatrix", ...
    "w", "v", "tpre", "delta_t", "txy", "amp", "b", "theta", "p", "mu");
